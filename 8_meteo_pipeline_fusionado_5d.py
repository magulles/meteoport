...
def download_copernicus_wave_data(points):
    ...
    # Horizonte largo de Copernicus: 7 días exactos en pasos horarios
...
def fetch_catalog_xml(catalog_url: str = PDE_CATALOG_XML) -> str:
...
def open_local_nc_from_url(url: str):
...
def download_pde_wave_data(points):
    ...
    for k, nc_name in enumerate(files, start=1):
        ...
        print(f"[{k}/{len(files)}] Descargando y procesando {nc_name}")
            ...
            if nearest_idxs is None:
                ...
                for point, (ilat, ilon) in zip(points, nearest_idxs):
                    ...
                    point_meta[point["point_id"]] = {
                        "lon": grid_lon,
                        "lat": grid_lat,
                        "distance_to_selected_grid_km": round(haversine_km(point["lon"], point["lat"], grid_lon, grid_lat), 3),
                    }
...
def merge_all_sources(copernicus_points, pde_points, wind_points):
    ...
    for idx, name in enumerate(all_names, start=1):
        ...
        if KEEP_ONLY_COMMON_TIMES:
            ...
            # Unión temporal: permite conservar Copernicus a 7 días y rellenar PDE con null tras 72 h
            selected_times = sorted(cop_times | pde_times | wind_times)
        ...
        for t in selected_times:
            ...
            merged_forecast.append({
                "time": t,
                # Copernicus: se mantienen las claves antiguas para compatibilidad web
                "hs": c.get("hs"),
                "tp": c.get("tp"),
                "di": c.get("di"),
                # PDE: nuevas claves explícitas
                "hs_pde": p.get("hs_pde"),
                "tp_pde": p.get("tp_pde"),
                "di_pde": p.get("di_pde"),
                # viento
