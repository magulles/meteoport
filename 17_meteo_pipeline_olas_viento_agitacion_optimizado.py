...
def classify_point(point: Dict) -> str:
    if is_port_point(point):
        return "port"
    if point_in_ibi_bbox(point["lon"], point["lat"]):
        return "ibi"
    return "glo"


def classify_points(points: List[Dict]) -> Dict[str, List[Dict]]:
    grouped = {"port": [], "ibi": [], "glo": []}
    for point in points:
...
# 1) COPERNICUS
# =========================
def point_in_ibi_bbox(lon: float, lat: float) -> bool:
    return (
        IBI_BBOX["lon_min"] <= lon <= IBI_BBOX["lon_max"]
        and IBI_BBOX["lat_min"] <= lat <= IBI_BBOX["lat_max"]
    )


def get_copernicus_dataset_for_point(lon: float, lat: float) -> str:
    if point_in_ibi_bbox(lon, lat):
        return COPERNICUS_DATASET_ID_IBI
    return COPERNICUS_DATASET_ID_GLO


def dataset_label(dataset_id: str) -> str:
    if dataset_id == COPERNICUS_DATASET_ID_IBI:
        return "ibi"
    if dataset_id == COPERNICUS_DATASET_ID_GLO:
        return "glo"
    return "unknown"
...
def fetch_copernicus_candidate(dataset_id, req_lon, req_lat, point_id, attempt_idx, start_datetime, end_datetime):
...
def _download_single_copernicus_point(point, search_plan, start_datetime, end_datetime):
    ...
    dataset_id = get_copernicus_dataset_for_point(base_lon, base_lat)

    print(
        f"\n[COPERNICUS PUNTO {point_id}] {name} | lon={base_lon}, lat={base_lat} | "
        f"dataset={dataset_label(dataset_id)}"
    )
    ...
    if best is None:
        return {
            "result": {
                ...
                "lat": None,
                "forecast": [],
                "error": f"No se pudo obtener ningún candidato de olas Copernicus ({dataset_label(dataset_id)})",
...
def merge_all_sources(copernicus_points, pde_points, wind_points, agitation_points):
    ...
    for idx, name in enumerate(all_names, start=1):
        ...
        if pde and "pde_error" in pde:
            merged_point["pde_error"] = pde["pde_error"]
        ...
        if wind and "source" in wind:
            merged_point["wind_source"] = wind["source"]
    ...
    summary = {
        ...
        "copernicus_dataset_ibi": COPERNICUS_DATASET_ID_IBI,
...
def main():
    ...
    port_points = classified["port"]
    ibi_points = classified["ibi"]
    glo_points = classified["glo"]
    offshore_points = ibi_points + glo_points

    print(f"Puntos puerto: {len(port_points)}")
    print(f"Puntos IBI: {len(ibi_points)}")
    print(f"Puntos GLO: {len(glo_points)}")
    print(
    ...
    if ibi_points:
        pde_points = download_pde_wave_data(ibi_points)

    wind_targets = offshore_points + port_points
    if wind_targets:
        wind_points = download_wind_data(wind_targets)
