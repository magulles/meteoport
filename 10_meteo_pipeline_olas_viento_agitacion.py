...
POINTS_FILE_AGITACION = "lonp_latp_agitacion.txt"
...
def fetch_catalog_xml(catalog_url: str, timeout=60, max_retries=4) -> str:
    last_error = None
...
def get_pde_region_inventory() -> Dict[str, Dict]:
    ...
    for region_key in PDE_REGION_PRIORITY:
        ...
        if not files:
            raise RuntimeError(f"La región PDE {region_key} no tiene ficheros horarios disponibles")
...
def download_pde_wave_data_for_region(points: List[Dict], region_meta: Dict):
    ...
    print(f"Run PDE más reciente detectado: B{region_meta['latest_run']}")
    ...
    for k, nc_name in enumerate(region_meta["files"], start=1):
        ...
        print(f"[{k}/{len(region_meta['files'])}] Descargando y procesando {nc_name}", flush=True)
...
def get_port_mesh_inventory() -> Dict[str, Dict]:
    ...
    for mesh_key in PORT_MESH_PRIORITY:
        ...
        if not files:
            raise RuntimeError(f"La malla portuaria {mesh_key} no tiene ficheros horarios disponibles")
...
def download_port_agitation_for_mesh(points: List[Dict], mesh_meta: Dict):
    ...
    print(f"Run más reciente detectado: B{mesh_meta['latest_run']}")
    ...
    for k, nc_name in enumerate(mesh_meta["files"], start=1):
        ...
        print(f"[{k}/{len(mesh_meta['files'])}] Descargando y procesando {nc_name}", flush=True)
...
def main():
    ...
    # NUEVO: leer puntos de agitación
    points_agitacion = []
    if Path(POINTS_FILE_AGITACION).exists():
        points_agitacion = read_points(POINTS_FILE_AGITACION)
        print(f"Se han leído {len(points_agitacion)} puntos de agitación")
    ...
    # NUEVO: descargar agitación
    agitation_points = download_port_agitation(points_agitacion) if points_agitacion else []
